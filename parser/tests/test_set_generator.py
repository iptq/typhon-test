...
from parser.models import SetGenerator
from parser.grammar import Grammar
from parser.symbols import *
from parser.orderedset import OrderedSet
...
class TestSetGenerator(object):
    class TestFirstSets(object):
        def test_direct(self):
            ...
            assert sg.first_of(GNT('S')) == OrderedSet(
                [GLiteral('a'), GLiteral('b')])
        ...
        def test_epsilon(self):
            sg = SetGenerator(Grammar.from_data(
                "S = 'a', B + 'c'\nB = 'b', EMPTY"))
            assert sg.first_of(GNT('S')) == OrderedSet(
                [GLiteral('a'), GLiteral('b'), GLiteral('c')])
            assert sg.first_of(GNT('B')) == OrderedSet(
                [GLiteral('b'), GEPSILON()])
            assert sg.first_of(GLiteral('a')) == OrderedSet([GLiteral('a')])
            assert sg.first_of(GLiteral('b')) == OrderedSet([GLiteral('b')])
...
""".strip()
            ...
            assert sg.get_first_sets() == dict([
                ("$accept", OrderedSet([GLiteral('a'), GLiteral(
                    'b'), GLiteral('c'), GLiteral('d'), GEPSILON()])),
                ("S", OrderedSet([GLiteral('a'), GLiteral('b'),
                                  GLiteral('c'), GLiteral('d'), GEPSILON()])),
...
""".strip()
            ...
            sg = SetGenerator(grammar)
            assert sg.follow_of(GNT('S')) == OrderedSet([GEOF()])
            assert sg.follow_of(GNT('A')) == OrderedSet(
                [GLiteral('b'), GLiteral('c')])
            assert sg.follow_of(GNT('A')) == sg.first_of_rhs(
                grammar.productions[1].right[1:])
...
""".strip()
            ...
            assert sg.follow_of(GNT('A')) == OrderedSet(
                [GLiteral('a'), GLiteral('b'), GLiteral('c')])
            assert sg.follow_of(GNT('A')) == sg.first_of_rhs(
                rhs[1:]).union(sg.first_of_rhs(rhs[3:]))
